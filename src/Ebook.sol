 ...
 *
 */
contract EbookStore is AccessControl, BucketApp, ObjectApp, GroupApp {
    /*----------------- constants -----------------*/
    // error code
    ...
    address public constant CROSS_CHAIN_EB = 0xa5B2c9194131A4E0BFaCbF9E5D6722c873159cb7;
    ...
    function initialize(
        ...
        bucketToken = CmnStorage(BUCKET_HUB_EB).ERC721Token();
        objectToken = CmnStorage(OBJECT_HUB_EB).ERC721Token();
        groupToken = CmnStorage(GROUP_HUB_EB).ERC721Token();
        memberToken = GroupStorage(GROUP_HUB_EB).ERC1155Token();

        __base_app_init_unchained(CROSS_CHAIN_EB, _callbackGasLimit, _failureHandleStrategy);
        __bucket_app_init_unchained(BUCKET_HUB_EB);
        __group_app_init_unchained(GROUP_HUB_EB);
        __object_app_init_unchained(OBJECT_HUB_EB);
    }
